...
class ResolvePage(WizardPage):
    ...
    def init_controls(self):
        ...
        tier_filter_widget = self.tier_filter_widget = QWidget()
        tier_filter_l = QHBoxLayout()
        tier_filter_widget.setLayout(tier_filter_l)
        tier_filter_label = QLabel(_('Filter by match algorithm'))
        ...
        tier_filter_l.addWidget(tier_filter_label)
        tier_filter_l.addWidget(tier_filter_combo)

        bll = QGridLayout()
        gbl.addLayout(bll)
        self.list_books_label = QLabel(_('Books in list:'), self)
        self.list_books_label.setStyleSheet('QLabel { font-weight: bold; }')
        self.matched_books_label = QLabel(_('Matches in library:'), self)
        self.matched_books_label.setStyleSheet('QLabel { font-weight: bold; }')
        bll.addWidget(self.list_books_label, 0, 0, 1, 1)
        bll.addWidget(self.matched_books_label, 0, 1, 1, 1)
        bll.addLayout(fl, 0, 2, 1, 1)
        bll.addWidget(tier_filter_widget, 1, 2, 1, 1)
        self.list_book_view = ListBookView(self)
        self.matched_book_view = MatchedBookView(self, self.db)
        bll.addWidget(self.list_book_view, 2, 0, 1, 1)
        bll.addWidget(self.matched_book_view, 2, 1, 1, 2)
        ...
        btnl.addWidget(self.update_metadata_button)
        btnl.addWidget(self.revert_metadata_button)
        btnl.addWidget(self.remove_book_button)
        btnl.addStretch(1)

        gb = QGroupBox(_('Possible matches for selected book:'), self)
    ...
    def initializePage(self):
        ...
        splitter_state = self.info['state'].get('resolve_splitter_state', None)
    ...
    def _refresh_filter(self):
        if self.filter_all.isChecked():
            ...
            self.tier_filter_widget.setVisible(False)
        elif self.filter_matched.isChecked():
            filter_criteria = FILTER_MATCHED
            self.tier_filter_widget.setVisible(True)
    ...
    def validatePage(self):
        ...
        self.info['resolve_splitter_state'] = bytearray(self.splitter.saveState())
        return True
